...
// ---------- safe inputs ----------
const page = usePage()
const user = page.props.auth?.user ?? { name: 'Student', status: 'pending' }
  ...
  <div class="rounded-xl border bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
    ...
    <div class="relative flex items-start justify-between gap-2">
      <div v-for="(s, idx) in steps" :key="s.key" class="relative flex-1">
        ...
        <div
          ...
          class="absolute left-1/2 top-5 h-px w-full -translate-x-1/2"
          :class="s.done ? 'bg-indigo-500/80' : 'bg-gray-300 dark:bg-gray-700'"
        ></div>

        <!-- Node (refined) -->
        <div class="relative z-10 flex flex-col items-center text-center">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-full border-2 text-sm font-semibold transition-all"
            :class="{
    ...
    <div class="mt-6">
      <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-800">
        <div
          class="h-1.5 rounded-full bg-gradient-to-r from-indigo-500 to-violet-600 transition-all duration-500 ease-out shadow-[inset_0_0_0_1px_rgba(0,0,0,0.02)]"
  ...
  <div class="rounded-xl border bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
    ...
    <div class="mt-4 grid gap-3 sm:grid-cols-3">
      ...
      >
        ...
        <span class="text-xs text-gray-500">{{ summary.unread_count ?? 0 }} unread</span>
      </Link>
    </div>
  </div>
</template>
