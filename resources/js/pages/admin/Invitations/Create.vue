...
interface Session { id: number | string; name: string; quota?: number; registered?: number }
...
const selectedSession = computed(() =>
    props.sessions.find(s => String(s.id) === String(form.convocation_session_id))
)
const left = (s?: Session) => {
    if (!s || typeof s.quota !== 'number' || typeof s.registered !== 'number') return undefined
    return Math.max(0, s.quota - s.registered)
}
const isFull = (s?: Session) => {
    const l = left(s)
    return typeof l === 'number' ? l <= 0 : false
}
    ...
    <AppLayout :breadcrumbs="[
        ...
        <div class="grid gap-6 md:grid-cols-3">
            ...
            <div class="md:col-span-2 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <div class="mb-3 flex flex-wrap items-center gap-2">
                    <div class="relative grow">
                        ...
                        <input v-model="q" type="text" placeholder="Search by name or email"
                    ...
                    <Button type="button"
                        ...
                        @click="selectAllFiltered" :disabled="!filtered.length"
                        title="Add all currently filtered results">
                        <Users class="h-4 w-4" />
                        Add all ({{ filtered.length }})
                    </Button>
                    <Button type="button" variant="ghost" class="h-9 px-3 text-gray-700 hover:bg-gray-100"
                        @click="clearAll" :disabled="!form.user_ids.length" title="Clear all selected recipients">
                        Clear
                    </Button>
                </div>
                    ...
                    <template v-if="form.user_ids.length">
                        <span v-for="id in form.user_ids" :key="id"
                            ...
                            <button type="button" class="ml-1 text-indigo-500 hover:text-indigo-700"
                                @click="removeOne(id)" aria-label="Remove recipient" title="Remove">
                                <X class="h-3.5 w-3.5" />
                            </button>
                        </span>
                    </template>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="s in filtered" :key="s.id" class="border-t"
                                ...
                                <td class="px-3 py-2">
                                    <input type="checkbox" :checked="isSelected(s.id)" @change="toggleId(s.id)"
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <select v-model="form.convocation_session_id"
                    ...
                    <option v-for="sess in props.sessions" :key="sess.id" :value="sess.id">
                        {{ sess.name }}
                        <template v-if="typeof sess.quota === 'number'">
                            — {{ (left(sess) ?? '—') }} left
                        </template>
                    </option>
                </select>
                <!-- Backend error for session selection -->
                <p v-if="form.errors.convocation_session_id" class="mt-2 text-sm text-red-600">
                    {{ form.errors.convocation_session_id }}
                </p>

                <!-- Full warning + override -->
                <p v-if="selectedSession" class="mt-2">
                    <span v-if="isFull(selectedSession)"
                        class="inline-block rounded bg-rose-100 px-2 py-1 text-xs text-rose-700">
                        Selected session is FULL — generation will be blocked unless override is enabled.
                    </span>
                </p>

                <label class="mt-3 flex items-center gap-2 text-sm">
                    <input type="checkbox" v-model="form.force" />
                    Allow override (issue even if session is full or student’s registration differs)
                </label>
                ...
                <div class="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-900">
                            {{ selectedSession?.name ?? 'Not set' }}
                        </span>
                    </div>
                    <div v-if="typeof selectedSession?.quota === 'number'" class="flex justify-between">
                        <span class="text-gray-600">Capacity</span>
                        <span class="font-medium text-gray-900">
                            {{ selectedSession?.registered ?? 0 }} / {{ selectedSession?.quota }} filled
                            <span v-if="left(selectedSession) !== undefined"> ({{ left(selectedSession) }} left)</span>
                        </span>
                    </div>
                </div>
                ...
                <div class="mt-6 flex flex-wrap gap-2">
                    <Button class="bg-indigo-600 text-white hover:bg-indigo-500"
                        :disabled="!form.user_ids.length || (!!selectedSession && isFull(selectedSession) && !form.force)"
                ...
                <!-- Generic server error surface (optional) -->
                <p v-if="form.errors.user_ids" class="mt-2 text-sm text-red-600">
                    {{ form.errors.user_ids }}
                </p>
                <p v-if="form.errors.force" class="mt-1 text-sm text-red-600">
                    {{ form.errors.force }}
                </p>
            </div>
        </div>
