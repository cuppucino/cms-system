...
<template>
  ...
  <AppLayout :breadcrumbs="[{ title:'Invitations', href: route?.('admin.invitations.index') ?? '#' }]">
    <div class="space-y-4 p-4">
      ...
      <!-- Flash message -->
      <div v-if="page.props.flash?.message" class="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm text-emerald-800">
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-for="inv in filtered" :key="inv.id" class="border-t hover:bg-gray-50">
              <td class="px-3 py-2">
                <img :src="`data:image/png;base64,${inv.qr_base64}`" alt="QR" class="h-12 w-12 rounded bg-white" />
              </td>
              ...
              <td class="px-3 py-2">
                ...
                <Button variant="ghost" class="ml-1 h-7 px-2 text-gray-600 hover:bg-gray-100" title="Copy code" aria-label="Copy code" @click="copyCode(inv.code)">
                  <Copy class="h-4 w-4" />
                </Button>
              </td>
              <td class="px-3 py-2">{{ inv.issued_at ?? '—' }}</td>
              <td class="px-3 py-2">
                <span v-if="inv.is_active" class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">
                  <CheckCircle2 class="h-3.5 w-3.5"/> Active
                </span>
                <span v-else class="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2 py-0.5 text-xs font-medium text-rose-700">
                  <ShieldOff class="h-3.5 w-3.5"/> Revoked
                </span>
              </td>
              <td class="px-3 py-2 text-right">
                <div class="inline-flex items-center gap-1">
                  <Button v-if="inv.is_active" variant="ghost" class="h-8 px-2 text-rose-600 hover:bg-rose-50" title="Revoke" aria-label="Revoke" @click="revoke(inv.id)">
