...
const props = defineProps<{
    course: { id: number; name: string; code: string; faculty?: string; convocation_session_id?: number | null },
...
const form = useForm({
    ...
    hood_color: props.course.hood_color || '',
    convocation_session_id: props.course.convocation_session_id ?? null
})
</script>
...
<template>
    ...
    <AppLayout
        :breadcrumbs="[{ title: 'Courses', href: route?.('admin.courses.index') ?? '#' }, { title: 'Edit', href: '#' }]">
        <div class="space-y-6 p-6 max-w-2xl">
            <h1 class="text-xl font-semibold text-gray-900">Edit Course</h1>

            <form @submit.prevent="form.put(route('admin.courses.update', props.course.id))"
