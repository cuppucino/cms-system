...
const props = defineProps<{ session: { id:number; date:string; start_time:string|null; end_time:string|null; location:string; quota:number; note?:string|null } }>()
...
const submit = () => form.put(route('admin.gown-sessions.update', { gown_session: props.session.id }))
</script>

<template>
  <Head title="Edit Gown Session" />
  <AppLayout :breadcrumbs="[{ title: 'Gown Sessions', href: route('admin.gown-sessions.index') }, { title:'Edit', href:'#' }]">
    <div class="space-y-6 p-6 max-w-3xl">
      <h2 class="text-xl font-semibold">Edit Gown Session</h2>
