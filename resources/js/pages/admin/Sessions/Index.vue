...
const totals = computed(() => {
    const t = { quota: 0, registered: 0, guest_quota: 0, guest_registered: 0 }
    for (const s of props.sessions || []) {
        t.quota += Number(s.quota || 0)
        t.registered += Number(s.registrations_count || 0)
        t.guest_quota += Number(s.guest_quota || 0)
        t.guest_registered += Number(s.guest_registered || 0)
    }
    return t
})
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                    ...
                    <div class="mt-3 h-1 w-full overflow-hidden rounded-full bg-gray-200">
                        <div class="h-1 rounded-full bg-gradient-to-r from-emerald-500 to-indigo-600"
                            :style="{ width: pct(totals.registered, totals.quota) + '%' }" />
                    </div>
                </div>
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                    <p class="text-sm text-gray-500">Total Guest Quota</p>
                    <p class="mt-1 text-2xl font-semibold text-gray-800">{{ totals.guest_quota.toLocaleString() }}</p>
                </div>
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                    <p class="text-sm text-gray-500">Total Guests Registered</p>
                    <p class="mt-1 text-2xl font-semibold text-gray-800">{{ totals.guest_registered.toLocaleString() }}
                    </p>
                </div>
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                    <p class="text-sm text-gray-500">Guest Fill Rate</p>
                    <p class="mt-1 text-2xl font-semibold text-gray-800">{{ pct(totals.guest_registered,
                        totals.guest_quota) }}%</p>
                    <div class="mt-3 h-1 w-full overflow-hidden rounded-full bg-gray-200">
                        <div class="h-1 rounded-full bg-gradient-to-r from-emerald-500 to-indigo-600"
                            :style="{ width: pct(totals.guest_registered, totals.guest_quota) + '%' }" />
                    </div>
                </div>
            </section>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-left text-gray-600">
                        <tr>
                            ...
                            <th class="px-4 py-3 font-medium text-right">Student Quota</th>
                            <th class="px-4 py-3 font-medium text-right">Students Registered</th>
                            <th class="px-4 py-3 font-medium text-right">Guest Quota</th>
                    ...
                    <tbody>
                        <tr v-if="!filtered.length">
                            <td colspan="8" class="py-8 text-center text-gray-500">No sessions available.</td>
                        </tr>
                        <tr v-for="s in filtered" :key="s.id" class="border-t hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-right tabular-nums">{{ s.guest_quota }}</td>
                            <td class="px-4 py-2 text-right tabular-nums">
                                <div class="inline-flex items-center justify-end gap-1">
                                    <Users class="h-4 w-4 text-gray-400" />
                                    <span>{{ s.guest_registered }}</span>
                                </div>
                            </td>
