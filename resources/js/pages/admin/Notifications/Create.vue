...
<template>
  ...
  <AppLayout :breadcrumbs="[{ title: 'Notifications', href: route('admin.notifications.index') }, { title: 'Send', href: '#' }]">
        ...
        <div class="md:col-span-2 space-y-3">
          ...
          <div class="max-h-64 overflow-auto rounded-xl border border-gray-200 bg-white shadow-sm">
            <div v-for="s in filtered" :key="s.id"
              ...
              <input type="checkbox" :checked="isSelected(s.id)" @change="toggle(s.id)" aria-label="Select recipient" />
              <div class="min-w-0 flex-1">
                <div class="truncate font-medium text-gray-900">{{ s.name }}</div>
                <div class="truncate text-xs text-gray-500">{{ s.email }}</div>
              </div>
              <Button type="button" variant="ghost"
                      class="h-7 px-2 text-xs text-indigo-700 hover:bg-indigo-50"
                      @click="toggle(s.id)">
          ...
          <div class="flex flex-wrap gap-2">
            <template v-if="form.user_ids.length">
              <span v-for="id in form.user_ids" :key="id"
                ...
                <button type="button" class="ml-1 text-indigo-500 hover:text-indigo-700" @click="removeOne(id)" aria-label="Remove recipient">
                  <X class="h-3.5 w-3.5" />
                </button>
              </span>
              <button type="button" class="text-xs text-gray-600 underline hover:text-gray-800" @click="clearAll">Clear all</button>
            </template>
