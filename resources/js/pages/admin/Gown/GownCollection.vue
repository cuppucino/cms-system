        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-for="c in props.collections" :key="c.id" class="border-t hover:bg-gray-50">
              ...
              <td class="px-4 py-2">
                <span v-if="c.status === 'collected'" class="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">Collected</span>
                <span v-else-if="c.status === 'returned'" class="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700">Returned</span>
                <span v-else-if="c.status === 'late'" class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">Late</span>
                <span v-else class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600">Pending</span>
              </td>
              <td class="px-4 py-2">{{ c.collection_date ?? '—' }}</td>
              <td class="px-4 py-2">{{ c.return_date ?? '—' }}</td>
              <td class="px-4 py-2 text-right">
                <div class="inline-flex items-center gap-1">
                  <Button variant="ghost" class="h-8 px-2 text-emerald-600 hover:bg-emerald-50" @click="updateStatus(c.id, 'collected')" title="Mark collected" aria-label="Mark collected"><CheckCircle2 class="h-4 w-4" /></Button>
                  <Button variant="ghost" class="h-8 px-2 text-blue-600 hover:bg-blue-50" @click="updateStatus(c.id, 'returned')" title="Mark returned" aria-label="Mark returned"><RotateCcw class="h-4 w-4" /></Button>
                  <Button variant="ghost" class="h-8 px-2 text-amber-600 hover:bg-amber-50" @click="updateStatus(c.id, 'late')" title="Mark late" aria-label="Mark late"><AlertTriangle class="h-4 w-4" /></Button>
                </div>
              </td>
            </tr>
          </tbody>
