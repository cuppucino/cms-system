        ...
        <header class="sticky top-0 z-40 border-b bg-white/70 backdrop-blur dark:bg-gray-900/70">
            <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
                ...
                <div class="flex items-center gap-2">
                    <span
                        class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 text-white">🎓</span>
                    <span class="font-semibold">Convocation</span>
                    <span
                        class="ml-2 rounded-full bg-indigo-50 px-2 py-0.5 text-xs text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200">Student</span>
                </div>

                <!-- Navigation -->
                <nav class="hidden gap-2 text-sm md:flex">
                    <Link v-for="item in nav" :key="item.href" :href="item.href"
                        class="rounded-lg px-3 py-2 transition hover:bg-gray-100 dark:hover:bg-gray-800" :class="isActive(item.href)
                ...
                <div class="relative flex items-center gap-4">
                    ...
                    <div class="relative" ref="notifMenuRef">
                        ...
                        <div v-if="showNotifMenu"
                            ...
                            <div class="max-h-60 overflow-y-auto p-2">
                                <div v-if="!user.recent_notifications || user.recent_notifications.length === 0"
                                    class="px-3 py-2 text-sm text-gray-500">
                                    No notifications yet.
                                </div>

                                <div v-for="n in user.recent_notifications" :key="n.id"
                                    ...
                                    <span class="text-xs text-gray-400">{{ n.created_ago }}</span>
                                </div>
                            </div>
                    ...
                    <div class="relative" ref="profileMenuRef">
                        <button @click="toggleProfileMenu"
                            ...
                            <span
                                class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-white">
