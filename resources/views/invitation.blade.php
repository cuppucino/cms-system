...
<body>
    ...
    <div class="panel">
        ...
        <div class="grid">
            ...
            <div class="value">{{ $invitation->user->name ?? '—' }}</div>
            <div class="label">Student ID</div>
            <div class="value">{{ $invitation->user->student_id ?? '—' }}</div>
            <div class="label">Course</div>
            <div class="value">{{ $invitation->user->course->name ?? '—' }}</div>
        </div>
    ...
    <div class="panel no-break">
        ...
        <div class="grid">
            ...
            <div class="value">{{ $invitation->session->name ?? 'N/A' }}</div>

            <div class="label">Date</div>
            <div class="value">{{ optional($invitation->session?->date)->format('d M Y') ?? 'TBA' }}</div>

            @if (isset($invitation->session->time))
                <div class="label">Time</div>
                <div class="value">{{ optional($invitation->session?->time)->format('h:i A') ?? 'TBA' }}</div>
            @endif

            <div class="label">Location</div>
            <div class="value">{{ $invitation->session->location ?? 'TBA' }}</div>

            <div class="label">Code</div>
            <div class="value font-mono">{{ $invitation->code ?? '—' }}</div>
        </div>

        <div class="note">Arrive at least <strong>30 minutes earlier</strong> for registration and gown collection.
        </div>
